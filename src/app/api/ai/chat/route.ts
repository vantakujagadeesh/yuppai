...
export async function POST(request: Request) {
  try {
    ...
    const messages = body.messages.filter((message: unknown): message is { role: "user" | "assistant"; content: string } => typeof message === "object" && message !== null && ["user", "assistant"].includes((message as { role?: string }).role ?? "") && typeof (message as { content?: unknown }).content === "string" && ((message as { content: string }).content.length <= 1200)).slice(-20);
