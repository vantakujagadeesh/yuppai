...
`/api/health` reports readiness for the catalog, AI, database, authentication, and playback providers separately. `/api/providers/catalog` is a server-only TMDB adapter for real movie search when `TMDB_API_KEY` or `TMDB_READ_TOKEN` is configured. `prisma/schema.prisma` defines the PostgreSQL persistence boundary for users, titles, watchlists, and watch events. The application deliberately reports `publishable: false` until all required providers are configured; this prevents a static catalog from being mistaken for a real OTT service.
